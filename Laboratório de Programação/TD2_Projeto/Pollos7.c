...
int main(void) {
    /* Tell libc that our input/output is ISO-8859-1 (Latin-1) */
    if (!setlocale(LC_ALL, "pt_PT.ISO-8859-1")) {
        /* Fallback to environment default if that locale isn’t installed */
        setlocale(LC_ALL, "");
    }
    ...
    for (i = 0; i < (size_t)N; ++i) {
        printf("%*d %s\n", width, word_counts[i].count, word_counts[i].word);
