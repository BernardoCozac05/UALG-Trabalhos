...
def t_tabela(gl, alpha):
    ...
    if gl > 30:
        gl = 30
    if gl not in tabela or alpha not in tabela[gl]:
        print("❌ Valor de t não disponível para gl =", gl, "e alpha =", alpha)
        return -1
    return tabela[gl][alpha]

def intervalo_confianca_media(n, media, s, tipo, alpha):
    gl = n - 1

    if tipo == "B":
        t_crit = t_tabela(gl, alpha / 2)
        li = media - t_crit * s / math.sqrt(n)
        ls = media + t_crit * s / math.sqrt(n)

    elif tipo == "US":
        t_crit = t_tabela(gl, alpha)
        li = -999999
        ls = media + t_crit * s / math.sqrt(n)
...
# B  - Bilateral
# US - Unilateral superior
# UI - Unilateral inferior
tipo = input("(B / US / UI): ").strip().upper()

print("Parâmetro a estimar:")
# M  - Média (μ)
# V  - Variância (σ²)
# DP - Desvio padrão (σ)
parametro = input("(M / V / DP): ").strip().upper()
...
s2 = float(input("s2: "))
#Nível de confiança
#confiança=99% -> alpha=0.01
conf = float(input("Confianca: "))
...
if parametro == "M":
    ...
    s = math.sqrt(s2)
    li, ls = intervalo_confianca_media(n, media, s, tipo, alpha)
elif parametro in ["V", "DP"]:
