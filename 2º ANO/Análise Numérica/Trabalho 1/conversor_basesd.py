...
def parse_input_number(num_str):
    """
    Analisa a string do número de entrada.
    Retorna uma tupla (sign, int_part_str, frac_part_str, sep_char).
    - sign: 1 para positivo, -1 para negativo.
    - int_part_str: dígitos da parte inteira (em forma de string, sem sinal).
    - frac_part_str: dígitos da parte fracionária (string vazia se não houver).
    - sep_char: o caractere do separador decimal usado ('.', ',' ou None se não houver parte fracionária).
    """
...
def validate_digits(int_part_str, frac_part_str, base):
    """
    Valida se todos os caracteres de int_part_str e frac_part_str são dígitos válidos na base especificada.
    Retorna True se válido; lança ValueError se encontrar dígito inválido.
    """
    max_value = base - 1
...
def convert_int_part_to_value(int_part_str, base):
    """
    Converte a parte inteira (string) da base especificada para um valor inteiro (base 10, tipo int Python).
    """
    value = 0
...
def convert_frac_part_to_fraction(frac_part_str, base):
    """
    Converte a parte fracionária (string) da base especificada para um Fraction representando seu valor.
    Retorna uma fração Fraction(numerator, denominator).
    """
    if frac_part_str == '':
...
def convert_value_to_base_int(value, base):
    """
    Converte um valor inteiro (base 10) para uma string na base especificada.
    Retorna a representação na base 'base' (sem sinal).
    """
    if value == 0:
        return '0'
    digits = []
...
def convert_value_to_base_frac(frac_fraction, base, max_digits=8):
    """
    Converte um valor fracionário (objeto Fraction) para uma representação em string na base especificada.
    Calcula até max_digits dígitos fracionários. Trunca se a representação não for exata dentro desse limite.
    Retorna a string dos dígitos fracionários (sem separador, apenas as casas decimais na base de destino).
    """
